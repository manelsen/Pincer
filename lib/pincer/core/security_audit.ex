defmodule Pincer.Core.SecurityAudit do
  @moduledoc """
  Security posture audit for local runtime configuration.

  Focused checks:
  - channel auth/token exposure
  - DM policy hardening
  - gateway bind risk
  """

  @type severity :: :ok | :warn | :error

  @type check :: %{
          id: atom() | {atom(), String.t()},
          severity: severity(),
          message: String.t(),
          meta: map()
        }

  @type report :: %{
          status: severity(),
          counts: %{ok: non_neg_integer(), warn: non_neg_integer(), error: non_neg_integer()},
          checks: [check()],
          config_path: String.t()
        }

  @default_config_file "config.yaml"
  @token_channels MapSet.new(["telegram", "discord", "slack"])
  @dm_channels MapSet.new(["telegram", "discord"])
  @risky_binds MapSet.new(["0.0.0.0", "::", "[::]"])

  @spec run(keyword()) :: report()
  def run(opts \\ []) do
    root = Keyword.get(opts, :root, File.cwd!())
    config_file = Keyword.get(opts, :config_file, @default_config_file)
    env_fetcher = Keyword.get(opts, :env_fetcher, &System.get_env/1)
    config_path = resolve_config_path(root, config_file)

    {config, checks} =
      case load_config(config_path) do
        {:ok, config, config_check} ->
          {config, [config_check]}

        {:error, check} ->
          {%{}, [check]}
      end

    checks =
      checks ++
        channel_auth_checks(config, env_fetcher) ++
        dm_policy_checks(config) ++
        gateway_bind_checks(config)

    counts = counts(checks)

    %{
      status: status_from_counts(counts),
      counts: counts,
      checks: checks,
      config_path: config_path
    }
  end

  defp resolve_config_path(root, config_file) do
    if Path.type(config_file) == :absolute do
      config_file
    else
      Path.expand(config_file, root)
    end
  end

  defp load_config(path) do
    cond do
      not File.exists?(path) ->
        {:error,
         error_check(
           :config_yaml,
           "Config file not found at #{path}",
           %{path: path, reason: :enoent}
         )}

      true ->
        case YamlElixir.read_from_file(path) do
          {:ok, config} when is_map(config) ->
            {:ok, config,
             ok_check(:config_yaml, "Config file loaded successfully", %{path: path})}

          {:ok, _other} ->
            {:error,
             error_check(
               :config_yaml,
               "Config root must be a map/object",
               %{path: path, reason: :invalid_root}
             )}

          {:error, reason} ->
            {:error,
             error_check(
               :config_yaml,
               "Config file is invalid YAML",
               %{path: path, reason: inspect(reason)}
             )}
        end
    end
  end

  defp channel_auth_checks(config, env_fetcher) do
    channels(config)
    |> Enum.filter(fn {_, cfg} -> enabled?(cfg) end)
    |> Enum.flat_map(fn {channel_name, cfg} ->
      token_env = read_field(cfg, :token_env)

      cond do
        present?(token_env) ->
          if present?(env_fetcher.(token_env)) do
            [
              ok_check(
                {:channel_auth, channel_name},
                "Token env #{token_env} is present",
                %{channel: channel_name, token_env: token_env}
              )
            ]
          else
            [
              error_check(
                {:channel_auth, channel_name},
                "Missing token env #{token_env} for enabled channel #{channel_name}",
                %{channel: channel_name, token_env: token_env}
              )
            ]
          end

        token_required_channel?(channel_name) ->
          [
            error_check(
              {:channel_auth, channel_name},
              "Enabled channel #{channel_name} requires token_env",
              %{channel: channel_name}
            )
          ]

        true ->
          []
      end
    end)
  end

  defp dm_policy_checks(config) do
    channels(config)
    |> Enum.filter(fn {name, cfg} -> enabled?(cfg) and dm_capable_channel?(name) end)
    |> Enum.map(fn {channel_name, cfg} ->
      dm_policy = read_field(cfg, :dm_policy)
      mode = normalize_dm_mode(dm_policy)

      case mode do
        :allowlist ->
          ok_check(
            {:dm_policy, channel_name},
            "DM policy uses allowlist mode",
            %{channel: channel_name, mode: "allowlist"}
          )

        :disabled ->
          ok_check(
            {:dm_policy, channel_name},
            "DM policy is disabled",
            %{channel: channel_name, mode: "disabled"}
          )

        :pairing ->
          ok_check(
            {:dm_policy, channel_name},
            "DM policy requires pairing",
            %{channel: channel_name, mode: "pairing"}
          )

        :open ->
          warn_check(
            {:dm_policy, channel_name},
            "DM policy is open and exposes direct-message surface",
            %{channel: channel_name, mode: "open"}
          )

        :missing ->
          warn_check(
            {:dm_policy, channel_name},
            "DM policy missing (fallback is effectively open)",
            %{channel: channel_name, mode: "missing"}
          )

        :invalid ->
          warn_check(
            {:dm_policy, channel_name},
            "DM policy mode invalid (fallback is effectively open)",
            %{channel: channel_name, mode: "invalid"}
          )
      end
    end)
  end

  defp gateway_bind_checks(config) do
    bind =
      read_nested_field(config, [:gateway, :bind]) ||
        read_nested_field(config, [:server, :bind]) ||
        read_field(config, :bind)

    if present?(bind) do
      normalized =
        bind
        |> to_string()
        |> String.trim()
        |> String.downcase()

      if MapSet.member?(@risky_binds, normalized) do
        [
          warn_check(
            :gateway_bind,
            "Gateway bind #{bind} exposes network surface to all interfaces",
            %{bind: bind}
          )
        ]
      else
        [ok_check(:gateway_bind, "Gateway bind is restricted", %{bind: bind})]
      end
    else
      []
    end
  end

  defp channels(config) when is_map(config) do
    case read_field(config, :channels) do
      channels when is_map(channels) ->
        channels
        |> Enum.map(fn {name, cfg} -> {normalize_channel_name(name), normalize_map(cfg)} end)

      _ ->
        []
    end
  end

  defp channels(_), do: []

  defp normalize_map(map) when is_map(map), do: map
  defp normalize_map(_), do: %{}

  defp token_required_channel?(name), do: MapSet.member?(@token_channels, name)
  defp dm_capable_channel?(name), do: MapSet.member?(@dm_channels, name)

  defp normalize_channel_name(name) do
    name
    |> to_string()
    |> String.trim()
    |> String.downcase()
  end

  defp normalize_dm_mode(nil), do: :missing

  defp normalize_dm_mode(dm_policy) when is_map(dm_policy) do
    dm_policy
    |> read_field(:mode)
    |> normalize_dm_mode_value()
  end

  defp normalize_dm_mode(_), do: :invalid

  defp normalize_dm_mode_value(nil), do: :missing
  defp normalize_dm_mode_value(""), do: :missing

  defp normalize_dm_mode_value(value) do
    case value |> to_string() |> String.trim() |> String.downcase() do
      "open" -> :open
      "allowlist" -> :allowlist
      "disabled" -> :disabled
      "pairing" -> :pairing
      _ -> :invalid
    end
  end

  defp enabled?(cfg) when is_map(cfg) do
    case read_field(cfg, :enabled) do
      true -> true
      "true" -> true
      _ -> false
    end
  end

  defp enabled?(_), do: false

  defp counts(checks) do
    %{
      ok: Enum.count(checks, &(&1.severity == :ok)),
      warn: Enum.count(checks, &(&1.severity == :warn)),
      error: Enum.count(checks, &(&1.severity == :error))
    }
  end

  defp status_from_counts(%{error: errors}) when errors > 0, do: :error
  defp status_from_counts(%{warn: warnings}) when warnings > 0, do: :warn
  defp status_from_counts(_), do: :ok

  defp read_field(map, key) when is_map(map) and is_atom(key) do
    Map.get(map, key) || Map.get(map, Atom.to_string(key))
  end

  defp read_field(_, _), do: nil

  defp read_nested_field(map, [single]), do: read_field(map, single)

  defp read_nested_field(map, [head | tail]) do
    case read_field(map, head) do
      next when is_map(next) -> read_nested_field(next, tail)
      _ -> nil
    end
  end

  defp present?(value) when is_binary(value), do: String.trim(value) != ""
  defp present?(value), do: not is_nil(value)

  defp ok_check(id, message, meta), do: %{id: id, severity: :ok, message: message, meta: meta}
  defp warn_check(id, message, meta), do: %{id: id, severity: :warn, message: message, meta: meta}

  defp error_check(id, message, meta),
    do: %{id: id, severity: :error, message: message, meta: meta}
end
