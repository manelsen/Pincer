defmodule Pincer.Project.DiagnosticTest do
  ...
  test "Generates ERROR_DIAGNOSTIC when max_retries is reached" do
    ...
    {:ok, _pid} = Server.start_link(id: id, session_id: "s-diag", objective: "Diagnostic test", max_retries: 1)
    ...
    Process.sleep(100) # Espera o worker subir
    
    # Resolvemos o PID da tupla :via antes de dar o send
    target_pid = GenServer.whereis(Pincer.Project.Registry.via_tuple(id))
    ...
    diagnostic_msg = Enum.find(messages, fn m -> String.contains?(m.content, "ERROR_DIAGNOSTIC") end)
